<template>
  <div class="fa-container">
    ...
    <el-alert title="FA: Forced Alignation on SRT, audio + SRT -> aligned SRT"></el-alert>
    ...
    <el-form label-width="100px">
      ...
      <el-form-item label="原始srt" required>
        <el-input v-model="form.srt" placeholder="请输入原始SRT, 包含文本" type="textarea" :autosize="{ minRows: 7, maxRows: 14 }"></el-input>
      </el-form-item>
      <el-form-item>
...
export default {
  ...
  methods: {
    submit() {
      ...
      this.$axios.post('/api/fa', { ...this.form, srt }).then(res => {
        if (res.data.task_id) {
    ...
    getResult() {
      ...
      this.timer = setInterval(() => {
        ...
        this.$axios.get(`/api/fa/${this.taskId}`).then(res => {
        ...
        this.$axios.get(`/api/fa/${this.taskId}/srt`).then(res => {
          this.result = res
          clearInterval(this.timer)
        }).catch(() => {
          this.limit += 1
